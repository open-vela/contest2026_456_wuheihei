...
本仓库提交一段真实 Codex CLI 只读审计会话，日志位于 `logs/bilibilidev/`。
...
4. `scripts/export_codex_contest_log.py` 对原始 Codex rollout 做确定性字段映射，再把规范化的临时 transcript 交给官方 `snapshot_core.py`。编号、脱敏、manifest 和最终 JSONL 均由官方采集核心生成，最终日志未被手工编辑。
...
- 采集模式：`cli`
- 官方校验结果：`ALL OK`
- 本次仅导出新建的比赛仓只读审计会话。更早的桌面会话不在赛事采集器支持范围内，且可能含历史凭证，因此未混入提交。

## 校验方法

在完整 OpenVela `repo` 工作区的队伍仓库目录执行：

```bash
python3 ../.claude/skills/contest-log-collector/tools/validate-log.py logs/
```

日志可证明 AI 参与了仓库审计、证据交叉核对和问题识别；它不代表项目全部历史会话，也不用于虚构代码生成比例。
