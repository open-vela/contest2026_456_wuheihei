...
当前提交以服务器最新真实开发板镜像所对应的 v10 源码为唯一基准。旧比赛代码、旧模型和旧指标没有混入本仓库。
...
- **可审计结果**：训练脚本、模型、混淆矩阵、各类别指标和 C 推理测试结果均随源码提交。
...
本次重新训练得到的模型与冻结模型 SHA256 完全一致。

## 六、Linux 主机 C 模拟器

这里重新编译当前 v10 的 C 特征提取、INT8 模型和三秒决策逻辑，不使用旧 Goldfish 镜像。
...
测试入口仅用于确认 C 推理链路和门限行为。随仓库提交的便利样本结果不是独立测试集，不能作为模型准确率；严格指标以 `results/python_cv/metrics_extended.json` 为准。

## 七、真实 OpenVela 开发板构建

目标硬件为润芯微 OpenVela 适配开发板，当前服务器构建配置对应 R528S3 Gemini S1。
...
- 生成时间：2026-09-18 10:45:09（服务器 UTC-04:00）
- 大小：113,143,844 字节
- SHA256：`046ae62216b8cdc00681471e58f8eb46837a5d4c399e7fd59fe364f3684658c2`

镜像体积是整个 OpenVela 固件大小，不是模型大小。编译产物没有提交到仓库。
...
INT8 路径采用权重对称量化、运行时反量化，主要收益是存储空间；本项目没有声明纯整数 MAC 加速。

## 九、可复现指标

评测规模为 390 条独立源录音、1,950 个一秒窗口。background 有 230 条源录音，其余四类各 40 条，因此必须同时报告总体和宏平均结果。
...
- INT8 的轻微上升是量化扰动造成的边界变化，正确结论是“本次五折未观察到精度损失”，而不是量化必然提升精度。
...
AI 参与了需求拆解、训练/交叉验证脚本完善、C/Python 一致性排查、板端问题定位、结果复核和提交材料整理。所有 AI 生成或修改的内容均经过源码检查和可执行验证。

官方 JSONL 日志只通过赛事日志归集工具导出；本仓不会使用手工伪造或 example 占位日志。日志目录规则见 [`logs/README.md`](logs/README.md)。
